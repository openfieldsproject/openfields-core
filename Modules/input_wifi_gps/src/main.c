...
static int mqtt_init(void) {
    ...
    while (mosquitto_connect(g_mosq, MQTT_HOST, MQTT_PORT, 60) != MOSQ_ERR_SUCCESS) {
        fprintf(stderr, "MQTT connect failed, retrying in 2s...\n");
        sleep(2);
    }
...
static void mqtt_publish(const char *line) {
    if(g_mosq) {
        int rc = mosquitto_publish(g_mosq, NULL, MQTT_TOPIC,
                                   (int)strlen(line), line, 1, false);
...
ssize_t read_n_bytes(int fd, void *buf, size_t n) {
    ...
    while(total < n) {
        ...
        if(r < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
            return -1; // treat timeout/disconnect as end
